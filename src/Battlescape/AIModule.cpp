		...
		if (_save->getTileEngine()->canTargetUnit(&origin, _aggroTarget->getTile(), &target, _unit, false))
		{
			...
			if (_save->getPathfinding()->getStartDirection() != -1)
			{
				...
				if (!waitIfOutsideWeaponRange && extendedFireModeChoiceEnabled)
				{
					...
					if (_attackAction.weapon && _attackAction.weapon->getRules()->isOutOfRange(distanceToTargetSq / 2.75f)) // make sure we can get the ruleset before checking the range
					{
